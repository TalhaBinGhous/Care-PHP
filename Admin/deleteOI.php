...
if (isset($_GET["id"]) && is_numeric($_GET["id"])) {
    ...
    if ($stmt->execute()) {
        ...
        header("Location: order-item.php?message=Item deleted successfully");
    } else {
        // Optionally, you can redirect with an error message
        header("Location: order-item.php?error=Could not delete item");
    }
    ...
    header("Location: order-item.php?error=Invalid item ID");
}
?>
